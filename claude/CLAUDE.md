...
Mes standards transverses vivent dans `~/dev/claude-setup`. Commandes : `/brancher` (mettre un projet aux normes), `/check` (diagnostiquer une dérive), `/revue` (review du code en profondeur), `/sans-ia` (retirer les marques d'IA), `/mcp` (brancher un service externe par projet), `/aide` (mémo des commandes et où ça marche). Agents spécialisés disponibles : `code-reviewer`, `security-auditor`, `test-writer`, `release-manager`.
